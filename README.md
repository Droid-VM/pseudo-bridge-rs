...
## Layout
    ...
    src/cli.rs        CLI (-i/-e/-m/-b/-fi/-fb/--nflog-group/--timeout/--max-cap/
                           --offload-workaround[-magic]/--arp-keepalive/--vmroute-table/--vmroute-rule)
...
## Run
    ...
    pbridge -i wlan0 -e ebpf -m fwd-with-offload --arp-keepalive 10   # Android Wi-Fi STA
...
**you** enslave up0 into a bridge. `-b yourbr` does that one enslave for you at session
init — and nothing more. Either way pbridge tracks the bridge (`fwd1.master` /
...
On Android Wi-Fi, `--arp-keepalive 10` keeps upstream v4 neighbour caches REACHABLE
from our side (unicast ARP replies + periodic GARP): the firmware ARP offload holds a
single v4 slot and drops other ARP requests in powersave (see ARCHITECTURE.md §ARP
keepalive).
